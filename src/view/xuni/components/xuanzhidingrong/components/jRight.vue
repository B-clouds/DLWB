...
export default {
  ...
  methods: {
    async areaInfoFn() {
      this.$axios
        .get(window.wgApiUrl + "/siteSelection/siteSelectionBasicInfo", {
          params: {
            areaId: "10000",
          },
        })
        .then((res) => {
          if (res.status == 200) {
            if (res.data.msg == "query success") {
              ...
              sessionStorage.setItem("xzdr_areaName", this.areaInfo.qymc);
            }
          }
...
.jr_t_t {
  ...
  display: flex;
  align-items: center;
  background: url("img/jr_t_t.png") no-repeat !important;
  background-size: 100% 100% !important;
}
.jr_t_t > .span1 {
