<template>
  <div class="buidan" v-if="show">
    ...
    <transition name="transitionLeft">
      <wuli
        v-show="show && showItem && !showLieBiao && showXX2 && navIndex == 0"
      ></wuli>
    </transition>
    <!-- 站房选址详情页面空间 -->
    <transition name="transitionRight">
      <kongjian
        @baoCun="baoCun"
        @lieBiao="lieBiao"
        v-if="show && showItem && !showLieBiao && showXX2 && navIndex == 0"
      ></kongjian>
    </transition>
    <!-- 站房选址详情页面间隔维护  真实模型左侧 -->
    <transition name="transitionLeft">
      <realLeft
        v-if="show && showItem && !showLieBiao && showXX2 && navIndex == 1"
      ></realLeft>
    </transition>
    <!-- 站房选址详情页面间隔维护  真实模型右侧 -->
    <transition name="transitionRight">
    ...
    <!-- 切换按钮 -->
    <transition name="transitionBottom">
      <div class="jjBtn" v-if="show && showItem && !showLieBiao && showXX2">
        <div
          class="jjBtnItem"
          @click="navClick(0)"
