    ...
    <div class="buidan">
      ...
      >
        ...
        <transition name="transitionLeft">
            <wuli
                @goNeiJing="goNeiJing"
                v-show="show && showItem && showXX2 && !showNeiJing"
            ></wuli>
        </transition>
        <!-- 杆塔布点详情页面空间 -->
        <transition name="transitionRight">
            <kongjian
                v-if="show && showItem && showXX2 && !showNeiJing"
            ></kongjian>
        </transition>
        <transition name="transitionLeft">
            <div
        ...
        <transition name="transitionLeft">
            <div
                v-show="show && showItem && !showNeiJing && showkaiguan"
        ...
        <transition name="transitionRight">
            <div
                class="fh_mr"
                v-show="show && showItem && !showNeiJing"
                @click="fhMrClick"
            ></div>
        </transition>
        <!-- 信息缩放按钮 -->
        ...
        <!-- 站房选址-自定义建站内景页面左侧 -->
        <transition name="transitionLeft">
            <zdyLeft2 v-show="show && showXX3 && showNeiJing"></zdyLeft2>
        </transition>
        <!-- 站房选址-自定义建站内景页面右侧 -->
        <transition name="transitionRight">
            <zdyRight2 v-show="show && showXX3 && showNeiJing"></zdyRight2>
        </transition>
...
import zdyLeft2 from "./components/zdyLeft2.vue";
import zdyRight2 from "./components/zdyRight2.vue";
export default {
    name: "d_zhongduan",
    components: {
        newLieBiao,
    ...
    methods: {
        ...
        // 内景返回外景
        fhMrClick2() {
            this.showNeiJing = false;
        },
        // 内景信息缩放按钮
        // 信息缩放按钮
        sfxxClick2() {
            this.showXX3 = !this.showXX3;
        },
        sfxxClick() {
        ...
        // 进入内景
        goNeiJing() {
            this.showNeiJing = true;
            let v2 = { ChooseType: "ShowCableTerminal", value: "true" };
            let data2 = {
                functionName: "Sundry",
                functionParameters: [
                    {
