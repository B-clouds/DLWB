    ...
    <div class="newLieBiao">
        ...
        <transition name="transitionLeft">
            <div class="tiaoJian" v-show="showTJ && showXX">
                ...
                <div class="tjBlock">
                    <div class="tops">所属电站</div>
                    <div class="bottoms">
                        <div class="inputs">
                            <div class="inputs">
                                <input placeholder="" v-model="search2" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="tjBlock">
                    <div class="tops">所属调度</div>
                    <div class="bottoms">
                        <div class="inputs">
                            <el-select
                                v-model="dd_value"
                                @visible-change="ddClick"
                                placeholder=""
                            >
                                <el-option
                                    v-for="item in dd"
                            ...
                            <img v-show="!dd_show" src="./img/xia.png" />
                            <img v-show="dd_show" src="./img/shang.png" />
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    mounted() {
        // 获取表头
        let v = {
            URL: "data/tableColumnInfo/" + 101 + "/" + 0 + "/" + 101 + "/" + 1,
            StructName: "获取站房间隔表头",
            verbType: "GET",
        };
        let data2 = {
            functionName: "WebServerMsg",
            backFunctionName: "getxlThZFJG",
            functionParameters: [
                {
                    key: "WebServerMsg",
        ...
        window.ue.interface.getxlThZFJG = this.getxlThZFJG;
    },
    methods: {
        getxlThZFJG(e) {
        ...
        hangClick(e) {
            let that = this;
            setTimeout(() => {
                that.$bus.$emit("hangObjs", e);
            }, 50);
            that.$emit("isShowItem", e.ssdz);
            let v = {
                FocusTarget: {
                    target: "HSInterval",
                    ID: e.ssdz,
        ...
        hang2(row, column, event) {
            // if (e) {
            //     this.$refs.mug18.toggleRowSelection(e);
            // } else {
            //     this.$refs.mug18.clearSelection(e);
            // }
            this.$refs.mug18.toggleRowSelection(row);
        },
        // 右键清除选中
        unHangClicks() {
            this.$refs.mug19.setCurrentRow();
        },
        hang2s(row, column, event) {
            if (this.showSelect) {
                this.$refs.mug19.toggleRowSelection(row);
                let arr = [];
                arr.push(row);
                let data = arr;
                let id = arr[0].oid;
