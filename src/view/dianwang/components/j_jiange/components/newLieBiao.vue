    ...
    <div class="newLieBiao">
        ...
        <transition name="transitionLeft">
            <div class="tiaoJian" v-show="showTJ && showXX">
                ...
                <div class="tjBlock">
                    <div class="tops">所属杆塔</div>
                    <div class="bottoms">
                        <div class="inputs">
                            <div class="inputs">
                                <input placeholder="" v-model="search3" />
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    mounted() {
        // 获取表头
        let v = {
            URL: "data/tableColumnInfo/" + 103 + "/" + 0 + "/" + 103 + "/" + 1,
            StructName: "获取架空杆塔间隔表头",
            verbType: "GET",
        };
        let data2 = {
            functionName: "WebServerMsg",
            backFunctionName: "getxlThGTBT",
            functionParameters: [
                {
                    key: "WebServerMsg",
        ...
        window.ue.interface.getxlThGTBT = this.getxlThGTBT;
    },
    ...
    methods: {
        ...
        hang2(row, column, event) {
            // if (e) {
            //     this.$refs.mug14.toggleRowSelection(e);
            // } else {
            //     this.$refs.mug14.clearSelection(e);
            // }
            this.$refs.mug14.toggleRowSelection(row);
        },
        // 右键清除选中
        unHangClick() {
            this.$refs.mug14.setCurrentRow();
        },
        hang2s(row, column, event) {
            if (this.showSelect) {
                this.$refs.mug15.toggleRowSelection(row);
                let arr = [];
                arr.push(row);
                let data = arr;
                let id = arr[0].oid;
