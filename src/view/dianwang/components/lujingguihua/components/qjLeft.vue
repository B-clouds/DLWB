<template>
  <div class="mxLeft" v-show="show">
    ...
    <div class="cxtj jdmx">
      ...
      <div class="jbBlock">
        <div class="jbBtn" v-show="!showAdd" @click="qxClick">
          <span>取消路径</span>
        </div>
        <div class="jbBtn" v-show="showAdd" @click="xzClick">
          <span>新增路径</span>
        </div>
        <div class="jbBtn jbBtn2" @click="bangdingClick">
          <span>保存路径</span>
        </div>
      </div>
    </div>
    <div class="zhezhao" v-show="showZZ">
      <div class="z_delete">
        <div class="z_d_block">
    ...
    <div class="zhezhao" v-show="showZZ2">
      <div class="z_delete">
        <div class="z_d_block">
          <img class="imgs" src="./img/jj2.png" />
          <span>路径名称已存在！！！</span>
  ...
  methods: {
    ...
    bangdingClick() {
      if (this.name.length < 1) {
        ...
        this.$axios
          .get(window.wgApiUrl + "/graphic/checkName", {
            params: {
              name: this.name,
            },
          })
          .then((res) => {
            ...
            if (newT == true) {
              ...
              ue.interface.broadcast("PSAPI", JSON.stringify(data));
              window.ue.interface.baocunss = this.baocunss;
            } else {
              this.showZZ2 = true;
              this.name = "";
            }
    ...
    qxClick() {
      ...
      ue.interface.broadcast("PSAPI", JSON.stringify(data));
      window.ue.interface.geshihua = this.geshihua;
    },
    // 监听点击新增
    xzClick() {
      let v = {
