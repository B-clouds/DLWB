<template>
  <div class="newLieBiao">
    ...
    <transition name="transitionLeft">
      <div class="tiaoJian" v-show="showTJ">
        <div class="tjTT">
          <span>条件选择</span>
        </div>
        <div class="new_block">
...
export default {
  ...
  methods: {
    ...
    hang2(row, column, event) {
      if (this.showSelect) {
        this.$refs.mug16.toggleRowSelection(row);
        let arr = [];
        arr.push(row);
        let data = arr;
        let id = arr[0].oid;
    ...
    // 右键清除选中
    unHangClick() {
      this.$refs.mug16.setCurrentRow();
    },
    hang2s(row, column, event) {
      if (this.showSelect) {
        this.$refs.mug17.toggleRowSelection(row);
        let arr = [];
        arr.push(row);
        let data = arr;
        let id = arr[0].oid;
    ...
    setCreates() {
      let node = this.selectList;
      let v = {
        range: { type: "Create", target: "MStationing", member: node },
      };
      let data = {
        functionName: "SaveGame",
    ...
    setDeletes() {
      let node = this.selectList;
      let v = {
        range: { type: "Delet", target: "MStationing", member: node },
      };
      let data = {
        functionName: "SaveGame",
