...
export default {
    ...
    methods: {
        bangdingClick() {
            // 1. 判断是否选中左侧信息
            if (window.gtBdJg2.oid != undefined || null || "") {
                // 2.判断是否选中右侧信息
                if (this.selectList1005.Str_Num != undefined || null || "") {
                    // 3.判断是否已绑定

                    if (window.gtBdJg2.bdgx == 0) {
                        // 未绑定,弹出是否确认弹框
                        this.showTSTK4 = true;
                    } else {
                        // 已绑定。弹出弹框提示先解绑
        ...
        // 绑定确认弹框
        TSTK4Click() {
            let datas = window.gtBdJg2;
            datas.bdgx = 1;
            datas["Str_ValID"] = this.selectList1005.Str_Side;
            let v = {
                TwinType: "Interval",
                ModifyType: "Update",
                Data: datas,
        ...
        gClick3(row) {
            ...
            if (this.selectList1005 != row) {
                this.selectList1005 = row;
                ...
                let v = {
                    ...
                    selectType: "true",
                    oid: this.cfOid,
                    selectNum: this.selectList1005.Str_Side,
                };
                let data = {
                    functionName: "IntervalFunc",
                    functionParameters: [
