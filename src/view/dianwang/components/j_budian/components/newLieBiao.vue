...
export default {
  ...
  mounted() {
    ...
    // 获取表头
    let v = {
      URL: "data/tableColumnInfo/" + 102 + "/" + 0 + "/" + 102 + "/" + 1,
      StructName: "获取架空杆塔布点表头",
      verbType: "GET",
    };
    let data2 = {
      functionName: "WebServerMsg",
      backFunctionName: "getxlThGTBD",
      functionParameters: [
        {
          key: "WebServerMsg",
    ...
    window.ue.interface.getxlThGTBD = this.getxlThGTBD;
  },
  methods: {
    // 获取表头
    getxlThGTBD(e) {
    ...
    hang2(row, column, event) {
      if (this.showSelect) {
        this.$refs.mug5.toggleRowSelection(row);
        let arr = [];
        arr.push(row);
        let data = arr;
        let id = arr[0].oid;
    ...
    // 右键清除选中
    unHangClick() {
      this.$refs.mug5.setCurrentRow();
    },
    hang2s(row, column, event) {
      if (this.showSelect) {
        this.$refs.mug6.toggleRowSelection(row);
        let arr = [];
        arr.push(row);
        let data = arr;
        let id = arr[0].oid;
    ...
    getDateLists(e) {
      ...
      ue.interface.broadcast("PSAPI", JSON.stringify(data));
      window.ue.interface.jd_setList = this.jd_setList;
    },
    // 获取数据列表回调
    jd_setList(e) {
    ...
    setCreates() {
      let node = this.selectList;
      let v = {
        range: { type: "Create", target: "Tower", member: node },
      };
      let data = {
        functionName: "SaveGame",
    ...
    setDeletes() {
      let node = this.selectList;
      let v = { range: { type: "Delet", target: "Tower", member: node } };
      let data = {
        functionName: "SaveGame",
        backFunctionName: "jd_delete",
        functionParameters: [
          {
            key: "range",
            value: JSON.stringify(v),
      ...
      ue.interface.broadcast("PSAPI", JSON.stringify(data));
      window.ue.interface.jd_delete = this.jd_delete;
    },
    jd_delete(e) {
      this.dataList = [];
      this.tableColumnList = [];
